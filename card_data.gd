...
const AUTHORS := {
	"poe": {"name":"Edgar Allan Poe", "title":"The House of Nevermore", "tag":"DREAD & RECURSION", "description":"Turn grief into pressure. Death draws fresh pages; the Raven drinks from a fading reputation.", "power":"Nevermore", "power_text":"Deal 2 damage to the opposing author.", "color":"9675bc", "bonus_tag":"dread", "bonus_stat":"attack", "passive_text":"Dread characters gain +1 attack on arrival."},
	"shelley": {"name":"Mary Shelley", "title":"The Galvanic Archive", "tag":"CREATION & SACRIFICE", "description":"Build enduring creations. Wounded characters grow furious, while careful restoration keeps them alive.", "power":"Galvanize", "power_text":"Your strongest character gains +1 attack and +2 health. If none, heal 3 reputation.", "color":"78a992", "bonus_tag":"creation", "bonus_stat":"health", "passive_text":"Creation characters gain +1 health on arrival."},
	"shakespeare": {"name":"William Shakespeare", "title":"The Thorned Stage", "tag":"AMBITION & TRAGEDY", "description":"Fill the stage, then raise every voice. Tragedy rewards daring attacks and an ensemble cast.", "power":"Raise the Curtain", "power_text":"All your characters gain +1 attack.", "color":"bc746b", "bonus_tag":"tragedy", "bonus_stat":"attack", "passive_text":"Tragedy characters gain +1 attack on arrival."},
	"austen": {"name":"Jane Austen", "title":"The Velvet Drawing Room", "tag":"WIT & RESILIENCE", "description":"Win through observation and endurance. Restore reputation, gather options, and outlast a reckless rival.", "power":"Social Insight", "power_text":"Heal 3 reputation and draw 1 card.", "color":"c3a66e", "bonus_tag":"resilience", "bonus_stat":"health", "passive_text":"Resilience characters gain +1 health on arrival."},
	"carroll": {"name":"Lewis Carroll", "title":"The Crooked Looking Glass", "tag":"WONDER & POSSIBILITY", "description":"Follow curiosity into unexpected combinations. Extra cards and inspiration make each turn a new puzzle.", "power":"Down the Rabbit Hole", "power_text":"Draw 2 cards.", "color":"75acbe", "bonus_tag":"wonderland", "bonus_stat":"health", "passive_text":"Wonderland characters gain +1 health on arrival."},
	"doyle": {"name":"Arthur Conan Doyle", "title":"The Baker Street Ledger", "tag":"OBSERVATION & DEDUCTION", "description":"Close in on the truth. Small, precise pressure and fresh evidence wear down a rival who cannot see the pattern yet.", "power":"The Science of Deduction", "power_text":"Deal 1 damage to the opposing author and draw 1 card.", "color":"6f8a9c", "bonus_tag":"deduction", "bonus_stat":"attack", "passive_text":"Deduction characters gain +1 attack on arrival."},
	"burroughs": {"name":"Edgar Rice Burroughs", "title":"The Wild Frontier", "tag":"SURVIVAL & ADVENTURE", "description":"Survive first, triumph after. Hardy characters shrug off wounds while the whole company grows tougher together.", "power":"Call of the Wild Frontier", "power_text":"All your characters gain +1 health.", "color":"a37c4a", "bonus_tag":"survival", "bonus_stat":"health", "passive_text":"Survival characters gain +1 health on arrival."}
}
...
## Persistent combat keywords: checked continuously while a unit is on the
## board (see GameState._hit_unit/_defeat_unit/_clash), as opposed to the
## one-shot triggers below.
const PERSISTENT_KEYWORDS := ["shield", "rage", "drain", "death_draw"]

## Translates a legacy single-keyword unit effect into an on_arrival trigger.
## Kept so every existing _card(...) call site can stay exactly as written;
## new cards can instead pass a richer "triggers" list via the extra dict.
static func _legacy_unit_triggers(effect: String) -> Array:
	match effect:
		"draw": return [{"event":"on_arrival","effects":[{"type":"draw","amount":1}]}]
		"inspire": return [{"event":"on_arrival","effects":[{"type":"gain_inspiration","amount":1}]}]
		"heal": return [{"event":"on_arrival","effects":[{"type":"heal_reputation","amount":2}]}]
		_: return []

## Translates a legacy single-keyword spell effect into an on_play trigger
## made of one or more composable effect primitives (see GameState._apply_effect).
static func _legacy_spell_triggers(effect: String) -> Array:
	match effect:
		"damage": return [{"event":"on_play","effects":[{"type":"damage_enemy_lane","amount":3}]}]
		"buff": return [{"event":"on_play","effects":[{"type":"buff_ally_lane","attack":2,"health":1}]}]
		"heal": return [{"event":"on_play","effects":[{"type":"heal_reputation","amount":3},{"type":"heal_ally_lane","amount":2}]}]
		"draw": return [{"event":"on_play","effects":[{"type":"draw","amount":2}]}]
		"foreshadow": return [{"event":"on_play","effects":[{"type":"gain_next_inspiration","amount":2},{"type":"draw","amount":1}]}]
		"weaken": return [{"event":"on_play","effects":[{"type":"weaken_enemy_lane","attack":2},{"type":"damage_enemy_lane","amount":1}]}]
		"expose": return [{"event":"on_play","effects":[{"type":"expose_lane","shield_break_damage":1,"author_damage":1}]}]
		"reprieve": return [{"event":"on_play","effects":[{"type":"grant_reprieve"}]}]
		"metaphor": return [{"event":"on_play","effects":[{"type":"repeat_last_spell"}]}]
		_: return []

## A simple, free default rarity curve based on cost so every card carries a
## usable value even before anyone hand-curates standouts via extra.rarity.
static func _default_rarity(cost: int) -> String:
	if cost >= 4: return "rare"
	if cost == 3: return "uncommon"
	return "common"

static func _card(id: String, title: String, author: String, cost: int, attack: int, health: int, effect: String, rules: String, lesson: String, source: String, extra: Dictionary = {}) -> Dictionary:
	var is_unit := author != "neutral" and not SUPPLEMENTAL_AUTHORS.has(author)
	var keywords: Array = [effect] if PERSISTENT_KEYWORDS.has(effect) else []
	var triggers: Array = _legacy_unit_triggers(effect) if is_unit else _legacy_spell_triggers(effect)
	var card := {"id":id,"name":title,"author":author,"cost":cost,"attack":attack,"health":health,"effect":effect,"type":"unit" if is_unit else "spell","kind":"Character" if is_unit else "Concept","text":rules,"lesson":lesson,"source":source,"supplemental":SUPPLEMENTAL_AUTHORS.has(author),"tags":[],"rarity":_default_rarity(cost),"keywords":keywords,"triggers":triggers}
...
static func _build_cards() -> Array[Dictionary]:
	return [
		_card("poe_raven","The Raven","poe",3,3,3,"drain","Direct strikes restore 1 reputation.","The repeated word 'Nevermore' changes meaning as the speaker's questions become more desperate.","The Raven (1845)",{"tags":["dread"]}),
		_card("poe_usher","Roderick Usher","poe",2,2,2,"death_draw","On defeat: draw 1 card.","The decaying house parallels the decline of the Usher family, linking setting and character.","The Fall of the House of Usher (1839)",{"tags":["dread"]}),
		_card("poe_montresor","Montresor","poe",3,4,2,"shield","Prevent the first damage this character takes.","Montresor's courteous language conceals revenge; his account invites doubts about his judgment.","The Cask of Amontillado (1846)",{"tags":["dread"]}),
		...
		_card("shelley_creature","The Creature","shelley",4,4,5,"rage","After surviving damage: gain +1 attack.","The Creature's account asks readers to examine how rejection and neglect shape a person.","Frankenstein (1818)",{"tags":["creation"]}),
		_card("shelley_victor","Victor Frankenstein","shelley",2,2,3,"inspire","On arrival: gain 1 inspiration.","Victor's pursuit of knowledge becomes dangerous when he abandons responsibility for his creation.","Frankenstein (1818)",{"tags":["creation"]}),
		...
		_card("shakespeare_hamlet","Hamlet","shakespeare",3,3,3,"death_draw","On defeat: draw 1 card.","Hamlet's soliloquies reveal competing impulses that public dialogue can conceal.","Hamlet (c. 1600–1601)",{"tags":["tragedy"]}),
		_card("shakespeare_macbeth","Lady Macbeth","shakespeare",2,3,2,"inspire","On arrival: gain 1 inspiration.","Lady Macbeth's early commands contrast with her later fragmented speech, revealing the effects of guilt.","Macbeth (c. 1606)",{"tags":["tragedy"]}),
		...
		_card("shakespeare_prospero","Prospero","shakespeare",4,4,4,"heal","On arrival: restore 2 reputation.","Prospero directs events like a dramatist, then chooses to give up his magic.","The Tempest (c. 1610–1611)",{"tags":["tragedy"]}),
		...
		_card("austen_darcy","Fitzwilliam Darcy","austen",4,3,6,"shield","Prevent the first damage this character takes.","Darcy's actions complicate the judgments formed from his reserved social manner.","Pride and Prejudice (1813)",{"tags":["resilience"]}),
		...
		_card("austen_anne","Anne Elliot","austen",2,1,4,"heal","On arrival: restore 2 reputation.","Anne's renewed relationship with Wentworth explores persuasion, constancy, and second chances.","Persuasion (1817)",{"tags":["resilience"]}),
		...
		_card("carroll_alice","Alice","carroll",2,2,3,"draw","On arrival: draw 1 card.","Alice questions strange rules, exposing the instability of language and authority in Wonderland.","Alice's Adventures in Wonderland (1865)",{"tags":["wonderland"]}),
		_card("carroll_cat","Cheshire Cat","carroll",3,3,3,"shield","Prevent the first damage this character takes.","The Cat's disappearing body turns a familiar grin into a playful challenge to ordinary logic.","Alice's Adventures in Wonderland (1865)",{"tags":["wonderland"]}),
		_card("carroll_rabbit","White Rabbit","carroll",1,1,2,"inspire","On arrival: gain 1 inspiration.","The Rabbit's anxiety about time draws Alice from the familiar world into the unexpected.","Alice's Adventures in Wonderland (1865)",{"tags":["wonderland"]}),
		_card("carroll_hatter","The Hatter","carroll",2,2,2,"draw","On arrival: draw 1 card.","The tea party treats Time as a person, turning a figure of speech into a comic situation.","Alice's Adventures in Wonderland (1865)",{"tags":["wonderland"]}),
		_card("carroll_queen","Queen of Hearts","carroll",4,5,3,"rage","After surviving damage: gain +1 attack.","The Queen's repeated threats satirize arbitrary power and the misuse of authority.","Alice's Adventures in Wonderland (1865)",{"tags":["wonderland"]}),
		_card("doyle_holmes","Sherlock Holmes","doyle",4,4,4,"draw","On arrival: draw 1 card.","Holmes reasons from small, overlooked details to a complete account, modeling inference as a reading skill.","A Study in Scarlet (1887)",{"tags":["deduction"]}),
		_card("doyle_watson","Dr. John Watson","doyle",2,2,4,"heal","On arrival: restore 2 reputation.","Watson's narration frames Holmes's brilliance for the reader, showing how a narrator shapes what audiences notice.","The Sign of Four (1890)",{"tags":["deduction"]}),
		...
		_card("burroughs_tarzan","Tarzan","burroughs",4,5,4,"rage","After surviving damage: gain +1 attack.","Raised outside human society, Tarzan lets the novel question which of his traits are natural and which are learned.","Tarzan of the Apes (1912)",{"tags":["survival"]}),
		_card("burroughs_carter","John Carter","burroughs",3,3,4,"shield","Prevent the first damage this character takes.","Carter's transportation to Mars lets the novel examine his Earth assumptions against an unfamiliar society.","A Princess of Mars (1912)",{"tags":["survival"]}),
		...
		_card("burroughs_tars","Tars Tarkas","burroughs",3,4,3,"death_draw","On defeat: draw 1 card.","Tarkas's loyalty complicates the Tharks' stated code of ruthlessness, showing character in tension with culture.","A Princess of Mars (1912)",{"tags":["survival"]}),
